...
def run(inputFilename, metricFilename, cv=3, neighbors=10, 
    ...
    if labeled:
        ...
        kf = model_selection .LabelKFold(labels, n_folds=cv)
    else:
        kf = model_selection.KFold(len(inputData), n_folds=cv)
    
    scores = model_selection.cross_val_score(rc, inputData, inputTarget, cv=kf, scoring="accuracy", n_jobs=jobs, verbose=verbose)
