...
class Token(object):
    ...
    def __str__(self): return str(self.s) # unicode has been replaced by str in python3
...
class Vocab(object):
    def __init__(self):
        self.tokens = set()
        self.strings = set()
        self.s2t = defaultdict(Token.not_found)  # string to tok
        self.i2t = defaultdict(Token.not_found)  # index to tok
        self.unk = None
        self.START_TOK = None
        self.END_TOK = None
    ...
    def pp(self, seq, delimiter=u''):
        return delimiter.join([str(self[item].s) for item in seq])
    ...
    def load_from_corpus(cls, reader, remake=False, src_or_tgt="src"):
        ...
        if not remake and os.path.isfile(vocab_fname):
            ...
            for item in reader:  # [sentence1, sentence2, ..., sentenceN]
                toklist = item
                for token in toklist:  # [<s>, word1, word2, ..., wordN, <e>]
                    v.add(token)
                count += 1
                if count % 10000 == 0:
                    print("...", count, end="")
...
def get_reader(name): # only parallel or bible
...
class BibleTranslationCorpus(CorpusReaderTemplate):
    ...
    def __iter__(self):
        ...
        if os.path.isdir(self.fname):
            ...
            for filename in filenames: # langpath
                with open(filename) as f:
                    doc = f.read()
                    for line in doc.split("\n"):
                        #if not line:  continue
